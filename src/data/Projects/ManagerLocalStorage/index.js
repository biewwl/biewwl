import Comparative from './images/comparative.jpg';
import With from "./images/with.jpg";
import Without from "./images/without.jpg";

const ManagerLocalStorage = {
  path: "manager-local-storage",
  name: "localStorage Manager",
  namePt: "Gerenciador de LocalStorage",
  cover: With,
  type: "Library",
  description:
    "This library, I developed to help those who use localStorage a lot. It automates and simplifies many processes involved!",
  descriptionPt:
    "Esta biblioteca, eu desenvolvi para aqueles que usam muito o localStorage. Feita para automatizar e simplificar muitos processos!",
  links: {
    repository: "https://github.com/biewwl/manager-local-storage",
    project: "https://www.npmjs.com/package/manager-local-storage",
  },
  images: [Comparative, Without, With],
  skills: ["Javascript"],
  tools: [],
  development: "individual",
  done: true,
  emphasis: 2,
};

export default ManagerLocalStorage;
